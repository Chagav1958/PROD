Настроить MCP-сервер rules-mcp на другом ПК. Пошаговая инструкция.

## Шаг 1: Исправить opencode.jsonc (если ошибка mcpServers)

Если при запуске OpenCode ошибка "Unrecognized key: mcpServers" —
в файле opencode.jsonc заменить ключ "mcpServers" на "mcp".

## Шаг 2: Записать файл server.py

Путь: C:\AIS\AI\rules-mcp\rules_mcp\server.py

Скопировать весь текст ниже и записать в этот файл (UTF-8 без BOM):

--- НАЧАЛО ФАЙЛА server.py ---
...
def get_rule(topic: str) -> str:
    """Возвращает краткую сводку правила по теме.
    Темы: кодировка, bash, ps5.1, preflight, task, сохр, pb, sql, vss, jira, pending, project."""
...
def get_full_rule(source: str) -> str:
    ...
    Источники: pb, sql, vss, jira, project, layout, entity, glossary, preflight, russian, pending, agents."""
...
def get_abbreviation(abbr: str) -> str:
    """Возвращает расшифровку сокращения проекта."""
    ...
    for k, v in abbrs.items():
        if clean.lower() in k.lower() or k.lower() in clean.lower():
            return f"{k} -- {v}"
    total = len(abbrs)
    return f"Сокращение '{clean}' не найдено. Всего: {total}."


@mcp.tool()
def preflight_check(file_path: str, operation: str = "write") -> str:
    """Preflight-проверка перед записью файла."""
    ext = Path(file_path).suffix.lower()
    lines: list[str] = []
    if ext in ENCODING_TABLE:
        info = ENCODING_TABLE[ext]
    ...
    if ext == ".ps1":
        ...
        lines.append("  - Нет ??, ??=, ?.")
        lines.append("  - Функции ДО вызова (Bug-041)")
    if ext in (".ps1", ".json", ".jsonc"):
        if "bash" in operation.lower() or "cmd" in operation.lower():
            lines.append("[BASH] powershell -NoLogo (без -NoProfile!)")
    if operation.lower() == "edit":
        lines.append("[EDIT] oldString != newString ? (Bug-039)")
    if not lines:
        lines.append(f"Нет специальных правил для '{ext}'.")
    return f"=== Preflight: {Path(file_path).name} ({operation}) ===\n" + "\n".join(lines)
...
1. Проверить синтаксис: python -c "import py_compile; py_compile.compile(r'C:\AIS\AI\rules-mcp\rules_mcp\server.py', doraise=True)"
2. Перезапустить OpenCode
3. Вызвать get_full_rule("sql") — должен вернуть полный текст sql-rules.mdc
