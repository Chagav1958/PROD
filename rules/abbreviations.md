...
| **PBSERV / СЕРВИС PB** | Управление 5 мерами защиты разработки PB/SQL (M1 enforcer+константы, M2 запрет bash-обхода, M3 линтер, M4 обязат. claude_*, M5 ждать MCP) | `PB-Service.ps1`; спека: `docs\services\pb-service.md` |
...
| **ПРОДОБР** | **ПРОД**олжение **ОБР**аботки по утверждённому плану. Команда — найти задачу в `pending-tasks.md` или `tasks/` со статусом «в работе» (чек-боксы [~] или [ ]) и продолжить выполнение с последней незавершённой контрольной точки. Применяется к любому утверждённому плану (текущему и будущим). |
