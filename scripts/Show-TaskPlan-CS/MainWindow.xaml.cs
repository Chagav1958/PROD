...
namespace Show_TaskPlan
{
    public partial class MainWindow : Window
    {
        ...
        private void RunAutoTestCommand()
        {
            ...
            try
            {
                switch (cmd)
                {
                    ...
                    case "click":
                        if (arg == "start") BtnStart_Click(this, new RoutedEventArgs());
        ...
        private void BtnStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            string gui = Path.Combine(sp, "Show-TaskPlanGUI.ps1");
            if (File.Exists(gui))
            {
                try
                {
                    ...
                    {
                        FileName = "powershell",
                        Arguments = $"-NoLogo -WindowStyle Hidden -File \"{gui}\"",
                        UseShellExecute = false
                    });
                    Log("INFO", $"Launched: {gui}");
                    Close();
                }
                catch (Exception ex) { Log("ERROR", $"Launch failed: {ex.Message}"); lblStatus.Text = $"Ошибка: {ex.Message}"; }
            }
            else
            {
                lblStatus.Text = "Show-TaskPlanGUI.ps1 не найден";
                Log("ERROR", "Show-TaskPlanGUI.ps1 not found");
            }
        }
